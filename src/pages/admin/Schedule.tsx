...
const getServiceInfo = (serviceId: string, services: any[]) => {
...
const AppointmentCard: React.FC<{
  ...
  services: any[];
  className?: string;
}> = ({ appointment, onClick, services, className = "" }) => {
  const serviceInfo = getServiceInfo(appointment.service_id, services);
  return (
    <button 
      onClick={onClick} 
      className={`block w-full text-left p-2 rounded border-l-4 text-xs hover:shadow-sm transition-shadow ${getStatusColor(appointment.status)} ${className}`}
    >
      <div className="font-semibold text-gray-900 truncate">{appointment.client_name}</div>
...
}> = ({ currentDate, appointments, services, statusFilter, onAppointmentClick }) => {
          ...
          <div className="flex flex-1 gap-4 p-2 relative">
            ...
            {getAppointmentsForHour(toLocalDateString(currentDate), hour).map((appt) => (
              <AppointmentCard 
                ...
                services={services}
                className="relative z-10 flex w-full max-w-[300px] flex-col rounded-lg border-l-4 p-3 text-xs hover:shadow-md transition-shadow text-left"
              />
            ))}
          </div>
        </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-1">
                {dayAppts.slice(0, 3).map((appt) => (
                  <AppointmentCard 
                    ...
                    onClick={() => onAppointmentClick(appt)} 
                    services={services}
                    className="block w-full text-left p-1.5 rounded text-xs hover:shadow-sm transition-shadow"
                  />
                ))}
                {dayAppts.length > 3 && (
...
const AppointmentModal: React.FC<{
  ...
  setIsRegisteringPayment: (value: boolean) => void;
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="mt-6 flex gap-3 pt-4 border-t border-gray-100">
            ...
            <button onClick={onComplete} className="flex-1 rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-500 flex items-center justify-center"><DollarSign className="mr-2 h-4 w-4" />Confirmar Pagamento</button>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="mt-6 flex gap-3 pt-4 border-t border-gray-100">
            ...
            <button onClick={onReschedule} className="flex-1 rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-500 flex items-center justify-center"><Save className="mr-2 h-4 w-4" />Confirmar</button>
          </div>
        </div>
      );
    }

    const serviceInfo = getServiceInfo(selectedAppointment.service_id, services);
        ...
        {selectedAppointment.status !== AppointmentStatus.COMPLETED && selectedAppointment.status !== AppointmentStatus.CANCELLED && (
          <div className="mt-6 flex flex-col gap-3 pt-4 border-t border-gray-100">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={onCancel} className="flex-1 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-50">Cancelar</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl animate-in fade-in zoom-in duration-200">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-full p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600"><X className="h-5 w-5" /></button>
        </div>
        {renderModalContent()}
      </div>
    </div>
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-8">
        ...
        <ScheduleFilters statusFilter={statusFilter} setStatusFilter={setStatusFilter} />

        {view === 'day' && <DayView currentDate={currentDate} appointments={appointments} services={services} statusFilter={statusFilter} onAppointmentClick={handleAppointmentClick} />}
        {view === 'week' && <WeekView currentDate={currentDate} appointments={appointments} services={services} statusFilter={statusFilter} onAppointmentClick={handleAppointmentClick} />}
        {view === 'month' && <MonthView currentDate={currentDate} appointments={appointments} services={services} statusFilter={statusFilter} onAppointmentClick={handleAppointmentClick} />}
        ...
        <AppointmentModal 
          ...
          onReschedule={handleSaveReschedule}
          onComplete={handleCompleteAppointment}
          onCancel={handleCancelAppointment}
          setIsRescheduling={setIsRescheduling}
          setIsRegisteringPayment={setIsRegisteringPayment}
